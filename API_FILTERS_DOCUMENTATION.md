...
| ---------------- | ------- | -------- | -------------------------- | -------------------------- |
| `per_page`       | integer | No       | Number of results per page | Integer (min: 1, max: 100) |
| `consent_status` | string  | No       | Consent status             | String (max 50 characters) |
| `legal_basis`    | string  | No       | Legal basis                | String (max 50 characters) |
| `from`           | date    | No       | Start date filter          | Date format: `YYYY-MM-DD`  |
| `to`             | date    | No       | End date filter            | Date format: `YYYY-MM-DD`  |
...
| Filter        | Type    | Required | Description                | Valid Values / Constraints                                                                 |
| ------------- | ------- | -------- | -------------------------- | ------------------------------------------------------------------------------------------ |
| `source_type` | string  | No       | Alert source type          | Enum: `kri`, `monitoring_rule`, `human_report`, `vendor_notice`, `security_tool`, `other`  |
| `from`        | date    | No       | Start date filter          | Date format: `YYYY-MM-DD` (must be before or equal to today)                               |
| `to`          | date    | No       | End date filter            | Date format: `YYYY-MM-DD` (must be before or equal to today, and after or equal to `from`) |
| `per_page`    | integer | No       | Number of results per page | Integer (min: 1, max: 100)                                                                 |
...
| Filter        | Type    | Required | Description                | Valid Values / Constraints                                                                                                                                                     |
| ------------- | ------- | -------- | -------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `action_type` | string  | No       | Type of action             | Enum: `kill_switch`, `rollback_release`, `key_rotation`, `blocklist_update`, `traffic_throttle`, `model_disable_tool`, `policy_change`, `communication`, `data_purge`, `other` |
| `from`        | date    | No       | Start date filter          | Date format: `YYYY-MM-DD` (must be before or equal to today)                                                                                                                   |
| `to`          | date    | No       | End date filter            | Date format: `YYYY-MM-DD` (must be before or equal to today, and after or equal to `from`)                                                                                     |
| `per_page`    | integer | No       | Number of results per page | Integer (min: 1, max: 100)                                                                                                                                                     |
...
| Filter       | Type    | Required | Description                | Valid Values / Constraints                                                                 |
| ------------ | ------- | -------- | -------------------------- | ------------------------------------------------------------------------------------------ |
| `rca_method` | string  | No       | Root cause analysis method | Enum: `5_whys`, `fishbone`, `timeline_analysis`, `fault_tree`, `other`                     |
| `from`       | date    | No       | Start date filter          | Date format: `YYYY-MM-DD` (must be before or equal to today)                               |
| `to`         | date    | No       | End date filter            | Date format: `YYYY-MM-DD` (must be before or equal to today, and after or equal to `from`) |
| `per_page`   | integer | No       | Number of results per page | Integer (min: 1, max: 100)                                                                 |
...
| Filter          | Type    | Required | Description                | Valid Values / Constraints                                                                    |
| --------------- | ------- | -------- | -------------------------- | --------------------------------------------------------------------------------------------- |
| `audience_type` | string  | No       | Notification audience type | Enum: `internal_exec`, `internal_staff`, `customers`, `regulator`, `vendor`, `media`, `other` |
| `channel`       | string  | No       | Notification channel       | Enum: `email`, `portal`, `status_page`, `phone`, `meeting`, `legal_letter`, `other`           |
| `from`          | date    | No       | Start date filter          | Date format: `YYYY-MM-DD` (must be before or equal to today)                                  |
| `to`            | date    | No       | End date filter            | Date format: `YYYY-MM-DD` (must be before or equal to today, and after or equal to `from`)    |
| `per_page`      | integer | No       | Number of results per page | Integer (min: 1, max: 100)                                                                    |
...
| Filter        | Type    | Required | Description                | Valid Values / Constraints                                                                 |
| ------------- | ------- | -------- | -------------------------- | ------------------------------------------------------------------------------------------ |
| `status`      | string  | No       | Action status              | Enum: `new`, `in_progress`, `blocked`, `pending_verification`, `closed`                    |
| `source_type` | string  | No       | Source type                | Enum: `incident`, `risk`, `feedback`, `override`, `audit`, `assessment`, `other`           |
| `priority`    | string  | No       | Action priority            | Enum: `low`, `medium`, `high`, `critical`                                                  |
| `from`        | date    | No       | Start date filter          | Date format: `YYYY-MM-DD` (must be before or equal to today)                               |
| `to`          | date    | No       | End date filter            | Date format: `YYYY-MM-DD` (must be before or equal to today, and after or equal to `from`) |
| `per_page`    | integer | No       | Number of results per page | Integer (min: 1, max: 100)                                                                 |
...
### Corrective Preventive Action Source Type
...
### Corrective Preventive Action Priority
...
### Incident Notification Audience Type
...
### Incident Root Cause Analysis Method
