...
import { SourceInfoStep } from "./steps/SourceInfoStep";
import { CAPADetailsStep } from "./steps/CAPADetailsStep";
import { AssignmentStep } from "./steps/AssignmentStep";
import { VerificationStep } from "./steps/VerificationStep";
import { CAPA_WIZARD_STEPS } from "../constants";

const initialFormData: CorrectivePreventiveActionFormData = {
...
const CreateCAPAWizard: React.FC = () => {
  const router = useRouter();
  const [currentStep, setCurrentStep] = useState(1);
  const [createCAPA, { isLoading }] =
    useCreateCorrectivePreventiveActionMutation();
  ...
  const handleFormSubmit = handleSubmit(
    async (data: CorrectivePreventiveActionFormData) => {
      try {
        ...
        const createData: CreateCorrectivePreventiveActionData = {
        ...
        await createCAPA(createData).unwrap();
      ...
      <Card className="p-6 border-[#E4E7EC] shadow-none">
        <div className="mb-6">
          <h1 className="font-sans font-semibold text-2xl tracking-normal text-[#1D2939]">
            Create New Corrective/Preventive Action
          </h1>
          <p className="font-sans font-normal text-sm tracking-normal text-[#667085] mt-1">
            Complete all steps to create your CAPA
          </p>
        </div>
...
export default CreateCAPAWizard;
