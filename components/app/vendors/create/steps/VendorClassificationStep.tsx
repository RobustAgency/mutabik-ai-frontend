...
export const VendorClassificationStep: React.FC = () => {
  const {
    register,
    setValue,
    watch,
      ...
      <div className="space-y-6">
        ...
        <div className="space-y-2">
          ...
          <p className="text-sm text-muted-foreground mb-4">
            Select all that apply. At least one type is required.
          </p>

          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 border border-gray-200 rounded-lg p-3">
            {VENDOR_TYPE_OPTIONS.map((type) => {
              const isSelected = types.includes(type.value as any);
              return (
                <label
                  key={type.value}
                  className={`flex items-center gap-2 text-sm p-2 border rounded-lg hover:bg-gray-50 cursor-pointer transition-colors ${
                    isSelected
                ...
                >
                  <input
                    type="checkbox"
                    className="rounded border-gray-300 text-[#039855] focus:ring-[#039855]"
                    checked={isSelected}
        ...
        <div className="space-y-2">
          ...
          >
            <SelectTrigger
              className={hasError("data_processing_role") ? "border-red-500" : ""}
            >
