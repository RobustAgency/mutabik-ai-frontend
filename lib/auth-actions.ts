...
export async function signup(formData: FormData) {
  ...
  // Redirect to verify email page with email parameter for resend functionality
...
export async function resendVerificationEmail(email?: string) {
  ...
  // If no email is provided, try to get it from the current session
  ...
  if (!emailToUse) {
    ...
    if (getUserError || !user?.email) {
      return {
        success: false,
        message: "No email provided and no authenticated user found",
      } as const;
    }
